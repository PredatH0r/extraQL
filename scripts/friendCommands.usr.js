...
(function () {
  function init() {
        ...
        fn: function(val) {
          ...
          if (friend >= 0) {
            if (quakelive.mod_friends.roster.fullRoster[friend].IsInvited()) {
              ...
              qz_instance.SendGameCommand('echo ' + val + ' is already your friend <3;');
            }
          } else if (!found) {
    ...
    window.OnCommNotice = function(error, json) {
      ...
      return oldOnCommNotice.call(this, error, json);
    }

    var oldOnCvarChanged = window.OnCvarChanged;
    window.OnCvarChanged = function(name, value, replicate) {
      var i;
      for (i in commands) {
      ...
      return oldOnCvarChanged.call(this, name, value, replicate);
    }

    var oldOnSubscribeRequest = window.IM_OnPresence;
    window.IM_OnPresence = function(json) {
      var msg = quakelive.Eval(quakelive.Eval(json).msg);
      if (msg) {
        qz_instance.SendGameCommand('echo ' + msg.FROM + ' sent you a friend invite.;');
      }
      return oldOnSubscribeRequest.call(this, json);
    }
  }
  init();
})();