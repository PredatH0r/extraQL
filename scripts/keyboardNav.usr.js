// ==UserScript==
// @id             186881
// @name           Quake Live Keyboard Navigation
// @version        0.16
// @namespace      phob.net
// @author         wn
// @description    Adds some browser navigation functionality to the QUAKE LIVE standalone client
// @include        http://*.quakelive.com/*
// @exclude        http://*.quakelive.com/forum*
// @run-at         document-end
// @updateURL      https://userscripts.org/scripts/source/186881.meta.js
// ==/UserScript==
...
!("Mousetrap" in window) && (function (J, r, f) {
...
// Pulled from http://www.codinghorror.com/blog/2006/02/standard-browser-keyboard-shortcuts.html

// Home
Mousetrap.bind("alt+home", function () { quakelive.Goto("home"); });

// Back
Mousetrap.bind(["alt+left", "backspace"], function () { history.back(); });

// Forward
Mousetrap.bind(["alt+right", "shift+backspace"], function () { history.forward(); });

// Refresh
Mousetrap.bind("f5", function () { location.reload(); });

// Refresh with cache purging (TODO: test if it works...)
Mousetrap.bind("mod+f5", function () { location.reload(true); });

// URL navigation
Mousetrap.bind("mod+l", function () {
  function escapeHandler(e) { if (27 === e.keyCode) cleanup(); }
  function cleanup() {
    $("#modal-input > input")[0].blur();
    $("#navprompt").jqmHide();
    $("body").off("keyup", escapeHandler);
  }

  qlPrompt({
    id: "navprompt"
    , title: "Navigate"
    , body: "<b>Warning:</b> A non-QUAKE LIVE URL will open your default browser."
    , showX: true
    , input: true
    , inputLabel: location.href
    , ok: function () {
      var url = $("#modal-input > input").val().trim();
      // If a QL URL, just navigate directly...
      if (/^https?:\/\/\w+\.quakelive\.com/.test(url)) {
        location.href = url;
      }
        // ... otherwise trigger opening in a new window
      else {
        if (0 !== url.indexOf("http:") && 0 !== url.indexOf("https:")) url = "http://" + url;
  ...
  setTimeout(function () {
    $("#modal-input > input")[0].select();
    $("body").on("keyup", escapeHandler);
  }, 0);
});
