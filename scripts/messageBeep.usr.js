...
contentEval(function (beep, icon, mute) {
  ...
  quakelive.mod_friends.roster.UI_OnRosterUpdated = function () {
    ...
    quakelive.mod_friends.node.find('#im-header span').css('top', '9px')
		...
		.contextMenu({
		  menu: 'GM_mbeepmenu', onBeforeShow: function (menu) {
		    var tmp = '';
		    for (i in prefText) {
		  ...
		  if (setting == 'useDefaultBeep') {
		    ...
		    var loaded;
		    qlPrompt({
		      title: 'Select file:', body: '<input id="GM_mbeepfile" type="file" accept="audio/*"><div id="GM_qlmb_fileinfo"></div>', ok: function () {
		        setBeep(loaded);
		        qlHidePrompt();
		      }
		    });
